        ...
        while Ok == 1 && sum(parent) <= u
            ...
            for q = (p-1):(-1):1
                ...
                if parent[Order[q]] == 0
                    ...
                    finder = reshape(find(x->(x==1),parent'),1,length(find(x->(x==1),parent')));
...
function GFunction( DataObj, X, PAX )
    ...
    while  d <= N
        ...
        global t1
        for t1 = 1:Integer(DimX)
            if RangeX[t1] == OriginalData[d,X]
                break
            end
        end
        Frequency[t1]  =  1;
      ...
      # test whether the class value in Sample[t] is the same as ClassValue or not. 
        ...
        # calulates the G-function score using the logarithmic gamma function. Used as a criterion for
